...
pub fn match_statement<'a>(constants: &'a BTreeMap<&str, Literal>) -> impl FnMut(&str) -> IResult<&str, Matcher, ParserError<&str>> + 'a {
    move |text| {
        let event_match_parser = {
            let content_parser =
                map_res::<_, _, _, _, ParserError<&str>, _, _>(literal_or_constant(constants), |event_code| {
        ...
        let producer_match_parser = {
            let content_parser =
                map_res::<_, _, _, _, ParserError<&str>, _, _>(literal_or_constant(constants), |producer_address| {
...
mod tests {
    ...
    fn producer_test() {
        let constants = BTreeMap::new();
        let (input, matcher) = match_statement(&constants)("match  producer  0xabab~u16;input").unwrap();
